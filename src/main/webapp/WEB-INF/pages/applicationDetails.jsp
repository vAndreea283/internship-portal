...
<t:pageTemplate pageTitle="Detalii aplicatie" activePage="applications">
    ...
    <h3>Interviu</h3>
    <c:choose>
        <c:when test="${pageContext.request.isUserInRole('MANAGE_OWN_APPLICATIONS')}">
        ...
        <c:otherwise>
            <p>Rezumat: ${empty interview ? 'Neinregistrat inca.' : interview.summary}</p>
            <p>Rezultat: ${empty interview ? '-' : interview.result}</p>
        </c:otherwise>
    </c:choose>
        ...
        <c:choose>
            <c:when test="${pageContext.request.isUserInRole('MANAGE_OWN_APPLICATIONS')}">
                <form class="mt-3" method="POST" action="${pageContext.request.contextPath}/SaveGrade">
                    <input type="hidden" name="application_id" value="${application.id}">
                    <div class="mb-3">
                        <label for="value" class="form-label">Nota (1-10)</label>
            ...
            <c:otherwise>
                <p>Nota: ${empty grade ? 'Neinregistrata inca.' : grade.value}</p>
            </c:otherwise>
        </c:choose>
    </c:if>
</t:pageTemplate>